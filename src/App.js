...
class Feed extends Component {
  ...
  render() {
    ...
    let feed = this.props.randomFeed;
    ...
    return (
      <div className="col-md-10 col-md-offset-1">
        <button className="btn btn-danger" onClick={this.props.randomizer}>Click me!</button>
        <hr className="divider"/>
...
class App extends Component {
  ...
  randomizer() {
    ...
    // We can simply use Math.floor() to create a random set of headlines, but
    ...
    this.setState({loader: "", randomFeed: randomArray});
    // Set state. When the button is clicked, set the state for randomFeed and the loader.
  }

  componentWillMount() {
  // Set your API URL with the API key.
  let url = `https://newsapi.org/v1/articles?source=ign&sortBy=top&apiKey=${apiKey}`;
...
render() {
  return (
    <div className="App row">
      ...
      <Feed randomFeed={this.state.randomFeed} loader={this.state.loader} randomizer={this.randomizer}/>
